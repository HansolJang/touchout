	...
	private void setUserProfileInfo() {
		NetworkManager.getInstance().getUserProfile(this, new OnResultListener<ProfileResult>() {
			...
			public void onSuccess(ProfileResult result) {
				if(result.success == 1) {
					facebookIDView.setText(result.result.items.name);
					profileView.setVisibility(ImageView.VISIBLE);
					imgLoader.displayImage(result.result.items.image, profileView);
				}
			}
