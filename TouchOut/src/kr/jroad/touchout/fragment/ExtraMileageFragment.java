		...
		NetworkManager.getInstance().getMileageData(getActivity(),
				new OnResultListener<MileageResult>() {
					...
					public void onSuccess(MileageResult result) {
						if (result.success == 1) {
							mileageView.setText(thousandFormat.format(result.result.mileage));
							
							//액티비티 상단의 마일리지 뷰 세팅
							((ExtraMyWalletActivity)getActivity()).setMileageView(thousandFormat.format(result.result.mileage));
						}
					}
